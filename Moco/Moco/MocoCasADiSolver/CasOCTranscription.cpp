...
void Transcription::createVariablesAndSetBounds(const casadi::DM& grid,
    ...
    m_constraints.endpoint.resize(m_problem.getEndpointConstraintInfos().size());
...
void Transcription::transcribe() {
    ...
    // --------------------
    // TODO: this is invoked twice! move body of setObjective() into this function?
    DM quadCoeffs = this->createQuadratureCoefficients();

    int numEndpointConstraints = (int)m_problem.getEndpointConstraintInfos().size();
