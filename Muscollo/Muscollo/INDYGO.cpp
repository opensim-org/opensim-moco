...
/// rather are coded separately.
template<typename T>
class INDYGOProblemSeparateIA : public tropter::OptimalControlProblem<T> {
public:
    const double activation_time_constantTODO = 0.015;
    const double deactivation_time_constantTODO = 0.060;
    INDYGOProblemSeparateIA(const INDYGO& mrs,
            const Model& model,
            const InverseMuscleSolverMotionData& motionData,
            const std::string& fiberDynamicsMode)
            : tropter::OptimalControlProblem<T>("INDYGO"),
        ...
        for (const auto& actuator : _model.getComponentList<Muscle>()) {
            ...
            // activation is possible with implicit muscle models and has
            // numerical benefits.
            this->add_control(actuPath + "_norm_activation_rate", {0, 1});
            // {actuator.get_min_control(),
            //  actuator.get_max_control()});
            // TODO use activation bounds, not excitation bounds (then
            // also update the property comment for zero_initial_activation).
            ...
            // These constraints are used to make sure the rate of change of
            // activation stays within what's possible (determined by
            // plugging in u = 0 and u = 1 into the activation dynamics
            // differential equation).
            // TODO use activation time constant from DeGroote class; move
            // the creation of those classes into this loop.
            this->add_path_constraint(actuPath + "_activation_rate_min",
                    {0, SimTK::Infinity});
            this->add_path_constraint(actuPath + "_activation_rate_max",
                    {-SimTK::Infinity, 1.0 / activation_time_constantTODO});
    ...
    void initialize_on_mesh(const Eigen::VectorXd& mesh) const override {
        ...
        // For caching desired joint moments.
        auto* mutableThis = const_cast<INDYGOProblemSeparateIA<T>*>(this);
    ...
    void calc_differential_algebraic_equations(
        ...
        for (Eigen::Index i_act = 0; i_act < _numMuscles; ++i_act) {
            ...
            // Unpack variables.
            const T& normActivRate = controls[_numCoordActuators + 2 * i_act];
            const T& activation = states[2 * i_act];

            // Activation dynamics.
            const double activRateMin = -1.0 / deactivation_time_constantTODO;
            const double activRateMax = 1.0 / activation_time_constantTODO;
            const T activationRate = activRateMin
                    + (activRateMax - activRateMin) * normActivRate;
            out.dynamics[2 * i_act] = activationRate;

            out.path[3 * i_act + 0] = activationRate
                    + activation / deactivation_time_constantTODO;
            out.path[3 * i_act + 1] = activationRate
                    + activation / activation_time_constantTODO;
            ...
            for (Eigen::Index i_act = 0; i_act < _numMuscles; ++i_act) {
                ...
                if (_useFiberLengthState) {
                    ...
                    _muscles[i_act].calcTendonForceStateEquilibriumResidual(
                            activation, musTenLen, musTenVel, normTenForce,
                            normTenForceRate,
                            out.path[3 * i_act + 2], tendonForces[i_act]);
                }
            }
        ...
        // Achieve the motion.
        // ===================
        out.path.segment(3 * _numMuscles, _numCoordsToActuate)
                = _desiredMoments.col(i_mesh).template cast<T>()
                - genForce;
    }
    void calc_integral_cost(const T& /*time*/,
...
INDYGO::Solution INDYGO::solve() const {
    ...
    OPENSIM_THROW_IF(get_activation_dynamics_mode() != "explicit" &&
            ...
            "'implicit'.");
    //OPENSIM_THROW_IF(get_activation_dynamics_mode() == "explicit", Exception,
    //        "Implicit activation dynamics is not supported yet.");

    // Create the optimal control problem.
    // -----------------------------------
    auto ocp = std::make_shared<INDYGOProblemSeparateIA/*TODO*/<adouble>>(*this,
    ...
    tropter::DirectCollocationSolver<adouble> dircol(ocp, "trapezoidal",
            "ipopt", numMeshPoints);
    //dircol.get_optimization_solver().set_max_iterations(4); // TODO
