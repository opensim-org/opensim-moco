...
void OpenSim::replaceJointWithWeldJoint(Model& model, 
    ...
    // If welding to ground, add an offset in the y-direction to the model is
    // visible.
    SimTK::Vec3 y_offset(0);
    if (parent_body_path.find("ground") != std::string::npos) {
        y_offset[1] = 1.2;
    }
    ...
    auto* new_joint = new WeldJoint(jointName,
        ...
        parent_offset->get_translation() + y_offset,
